...
class AssetHelper
{
	 ...
	 * @return String
	**/
	public static function getPath(?asset:String, ?type:CrowbarAsset):String
	{
		final gottenPath:String = type.getExtension('assets/${asset}');
		#if MODS
	 ...
	 * @return Dynamic
	**/
	public static function getAsset(asset:String, ?type:CrowbarAsset):Dynamic
	{
	 ...
	 * @return Dynamic
	**/
	public static function parseAsset(asset:String, ?type:CrowbarAsset):Dynamic
	{
	...
	**/
	public static function getGraphic(file:String, ?customKey:String = null, ?vram:Bool = true):FlxGraphic
	{
		try
		{
			...
			try
			{
				final bd:BitmapData = #if (sys && !hl) OptimizedBitmapData.fromFile(file,
					vram) #elseif hl BitmapData.fromFile(file) #else OpenFLAssets.getBitmapData(file) #end;
	...
	**/
	...
	public static function sendToCache(key:String, asset:Dynamic, type:CrowbarAsset):Void
	{
		switch type
		{
			case IMAGE:
...
**/
enum abstract CrowbarAsset(String) to String
{
	var IMAGE = "image";
	var VIDEO = "video";
	var SOUND = "sound";
