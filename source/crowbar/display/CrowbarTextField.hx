...
**/
class CrowbarTextField extends FlxObject
{
	...
	public var borderType(default, set):CrowbarTextBorder = NONE;
	...
	/**
	 * Creates a new `CrowbarTextField` object at the specified position.
	...
	**/
	...
	public function setFormat(font:String, size:Int, color:Null<FlxColor> = 0xFF000000, align:TextFormatAlign = LEFT, borderType:CrowbarTextBorder = NONE,
	...
	function set_borderType(newBorder:CrowbarTextBorder):CrowbarTextBorder
	{
	...
	function set_borderSize(newSize:Float):Float
	{
		...
		if (Std.isOfType(borderType, OUTLINE))
			...
			trace('[CrowbarText:set_borderSize]: Can\'t set borderSize with the current border type ("${borderType}"), please use the OUTLINE type text border.');
		#end
		return borderSize;
	}
